...
exports.teacherregister = async(req,res)=> {
    try{
         ...
         try{
            let teacher= await Teacher.findOne({temail:teacherfields.temail})
            if(!teacher){
    ...
    try{
        ...
        if(isMatch){
            ...
            jwt.sign(payload, config.get('secretKey'), (err, token) => {
                if (err)
                    throw err;
                const loggedteacher = { teacherid: teacher.id,teachername: teacher.tname, teacheremail: teacher.email };
